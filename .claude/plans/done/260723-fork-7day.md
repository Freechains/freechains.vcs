...
| `tst/fork-7-days.lua` | new file           | rule 1 flips ordering vs prefix reps (time axis)   |
| `tst/fork-100-posts.lua` | new file        | rule 1 flips ordering vs prefix reps (post axis)   |
...
#### Correction: reps cap forbids stockpiling

`C.reps.max = 30` caps every author, so "farm 300 reps" is
impossible. The poster must **cycle** reps instead:

| fact                                  | source              |
|---------------------------------------|---------------------|
| author reps capped at 30              | `constants.lua:18`  |
| free posts require poster set >= 50%  | `common.lua:103`    |
| >= 50% share also wins rule 2         | `consensus` in sync |

Free posting and losing on reps therefore remain mutually
exclusive — but that does not block the test, because the
12h discount refund is **not rate-limited**. A broke poster
holds a negligible share, so its discount is the full 12h,
giving a sustainable rate of `reps / 12h`.

With GEN_3 (10 reps each) that is 1 post per 1.2h; posting
every 1.5h yields 100 posts in 150h, under the 168h time
threshold — so the post axis is isolated.

#### Decided construction (as implemented)

```
GEN_3: KEY1=10, KEY2=10, KEY3=10
fork point = genesis

    branch A: KEY3 x100 posts, 1.5h apart  (prefix reps 10)
    branch B: KEY1 x1, KEY2 x1             (prefix reps 20)

A recvs from B:
    without rule 1 -> B first (20 > 10)
    with    rule 1 -> A first (posts >= 100)
```

Distinct posts, no content conflict: all 102 survive, so
the assertion is on **ordering** — `list order` yields A's
100 posts, then Q1, Q2.

No rep farming is needed: the poster starts at the pioneer
share and cycles it.
...
3. ~~Write `fork-100-posts.lua`.~~ DONE, passing
4. ~~Run it.~~ DONE — full suite green
