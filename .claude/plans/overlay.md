...
Each host stores its known peers in:

```
.freechains/local/peers.lua
```

```lua
return {
    ["192.168.1.10:8330"] = { reps=5 },
    ["freechains.org:8330"] = { reps=true },
    ["10.0.0.99:8330"] = { reps=false },
    ["203.0.113.42:8330"] = { reps=12 },
}
```

This file is synced between owner peers via `config/`
(git push/pull), so all owner nodes share the same
peer list and reputation state.
...
- [peers.md](peers.md) — Peer identity, coalitions
- [network.md](network.md) — Transport options
