...
### Files to edit (pending, NOT started)

| file            | place                    | change                                            |
|-----------------|--------------------------|---------------------------------------------------|
| `constants.lua` | return table, new `fork` | `fork = { time = 7*24*h, posts = 100 }`           |
| `sync.lua`      | near `consensus` (~l.78) | add `hardfork(com, tip)` helper                   |
| `sync.lua`      | line 261                 | wrap consensus call with the `hardfork(oct,loc)` pre-check |
...
### fork-7-days.lua (ready to write)

Mirror `tst/consensus.lua` test 2, but flip the outcome:

- `GEN_2`; make KEY1 > KEY2 at fork (KEY2 likes seed).
- A posts with KEY2 (lower reps) at `--now` >= fork +
  7 days (604800 s).
- B posts with KEY1 (higher reps) at a young `--now`.
- A recvs from B.
- Without rule 1: B wins (higher reps). With rule 1:
  A's branch crossed 7 days → A wins.
- Assert A's file keeps KEY2's post, discards KEY1's.

### fork-100-posts.lua — BLOCKED on a decision

Structural finding: posting capacity is tied to
reputation. An author posts "for free" (instant refund,
unlimited same-instant posts) only when `discount = 0`,
which requires holding >= 50% of prefix rep:

```
discount = half * max(0, 1 - 2*ratio)   -- ratio = poster share
discount = 0  <=>  ratio >= 0.5  <=>  poster holds >= 50% rep
```

But rule 1 is only meaningful in a test when the poster
LOSES on reps (< 50%), so rule 1 flips the outcome. These
are mutually exclusive:

> An author who loses on reps cannot make 100 posts
> without time passing; 100 refund-delayed posts drag the
> branch past 7 days, so the 7-day rule fires first. A
> clean "100 posts, < 7 days, poster loses on reps"
> scenario is essentially unconstructible with honest
> posts.

**OPEN QUESTION (decide before writing fork-100-posts.lua):**
pick how the 100-post test isolates the post threshold.

| Option              | Setup                                                        | Isolates posts < 7d? |
|---------------------|--------------------------------------------------------------|----------------------|
| 1. Equal-rep tie    | Poster at exactly 50% posts 100 free; remote ties on reps; rule 1 breaks the tie for the 100-post branch | yes, via the tie path |
| 2. Both fire        | Poster accumulates over weeks, posts 100 across > 7 days     | no — 7-day also fires |
| 3. Env override     | `constants.lua` reads `FC_FORK_POSTS`; test sets it to 3     | yes, clean test-only knob |

Note: option 1 relies on the hash-tiebreak path, which is
not directly controllable (depends on commit hashes) — may
need the assertion phrased as "the 100-post branch wins
regardless of tiebreak".

### Next steps (explicit, in order)

1. ANSWER the open question above (option 1 / 2 / 3).
2. Implement the three code edits in "Files to edit".
3. Write `fork-7-days.lua`.
4. Write `fork-100-posts.lua` per the chosen option.
5. Run both tests (ask the user; do not self-run).
6. If option 3 chosen, wire `FC_FORK_POSTS` /
   `FC_FORK_TIME` reads into `constants.lua`.
