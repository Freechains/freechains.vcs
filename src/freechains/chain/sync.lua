...
elseif ARGS.recv then
    do
        ...
        -- Verify everything the fetch brought in, BEFORE anything reads
        -- it. Every check is a fact about the commit alone, so this is a
        -- set difference against the frontier, not a traversal, and each
        -- commit costs it once ever.
        --
        -- It has to run here, ahead of `G_oct`: that read is the whole
        -- point. `oct` is a common ancestor, so the old code took it on
        -- trust -- but local history is not all ours (a fast-forward
        -- makes their branch ours, a merge brings the loser's commits in
        -- as parent 2, a clone validates nothing), and the trust was
        -- circular. Now the premise is earned.
        --
        -- Begs are not reachable from `main`: they sit on their own refs
        -- until a positive `like` merges them in, so they are their own
        -- roots for the set difference.
        do
            local tips = { rem }
            local out = exec {
                cmd = "git -C " .. REPO .. " for-each-ref refs/begs/ --format='%(objectname)'"
            }
            for beg in out:gmatch("%x+") do
                tips[#tips+1] = beg
            end
            for _, tip in ipairs(tips) do
                local ok, err = pcall(verify, tip)
                if not ok then
        ...
        if ff then
            ...
            -- no state comparison here any more: `verify` already
            -- checked every commit the fast-forward brings in, tip AND
            -- interior, which is strictly more than this ever did
            goto RECV
        end
        ...
        do
            ...
            for _, hash in ipairs(O_snd) do
                ...
                -- the LOSER's commits, replayed onto the winner: only
                -- the order-dependent half, `check` ran long before
                ok, err = pcall(play, G_fst, hash, nil)
                if not ok then
                    goto DONE
                end
    ...
    -- advance the frontier: what we just checked, plus the commits we
    -- built ourselves out of it
    frontier("HEAD")
end
