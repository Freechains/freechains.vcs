...
-- `chain abandon <hash>` abandons `hash` and everything after it, the only
...
-- A post is always committed on top of a valid tip, so `abandon` lands on
...
--            p1[K1]
--              |
--            p2[K1]        <-- abandon target, abandoned
--              |
--            p3[K1]        <-- abandoned
--
-- State reverts with the tree, so the abandoned posts never happened: the
...
--            p1[K1] -------- b1[K2]
--
-- A beg is a post with nothing after it, so `abandon b2` just deletes its
-- ref and leaves `main` alone.
-- `abandon p1` lands on G, so b1 can no longer attach to main and its ref
...
--
-- After `abandon a1`, A holds no exclusive history left to protect, so
...
local ROOT_A = ROOT .. "/cli-abandon/A/"
local ROOT_B = ROOT .. "/cli-abandon/B/"
local EXE_A  = ENV .. " ../src/freechains.lua --root " .. ROOT_A
local EXE_B  = ENV .. " ../src/freechains.lua --root " .. ROOT_B

-- 1. Drop local posts back to a previous one
do
    print("==> Test 1: abandon removes posts")
    ...
    do
        ...
        assert(#T == 2, "expected 2 abandoned, got " .. #T)
        assert(T[1] == p2, "p2 should be abandoned first")
        assert(T[2] == p3, "p3 should be abandoned second")
    end
    ...
    -- `abandon` names the first post to remove, so it lands on its
...
do
    ...
    do
        TEST "abandon a beg deletes its ref alone"
        local head = exec {
            cmd = "git -C " .. DIR2 .. " rev-parse HEAD",
        }
        local out = exec {
...
-- 3. Hard fork recovery: abandon, recv, repost, back in sync
do
    ...
    do
        ...
        assert(#T == 2, "expected 2 abandoned, got " .. #T)
        assert(T[1]==a1 and T[2]==a2, "a1 and a2 should be abandoned")
    end
