...
exec(ENV_EXE .. " chains add cli-reps config " .. GEN_1)
...
do
    ...
    exec(ENV_EXE .. " chains add cli-reps config " .. GEN_2)
...
do
    ...
    do
        ...
        exec (
            ENV_EXE .. " chains add cli-reps config " .. GEN_2
        )
        local out1 = exec (
            ENV_EXE .. " chain cli-reps reps author " .. KEY
        )
    ...
    do
        ...
        exec("rm -rf " .. TMP)
        exec("mkdir -p " .. ROOT)
        exec(ENV_EXE .. " chains add cr7 config " .. GEN_3)
        local out = exec (
